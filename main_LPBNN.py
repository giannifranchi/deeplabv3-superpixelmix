...
num_models=4
def tile(x,n_tile):
    return torch.cat([x for i in range(n_tile)], dim=0)
def get_argparser():
    ...
    # Deeplab Options
    parser.add_argument("--model", type=str, default='deeplabv3plus_VAE_resnet101',
...
def validate(opts, model, loader, device, metrics, ret_samples_ids=None):
    ...
    with torch.no_grad():
        for i, (images, labels) in tqdm(enumerate(loader)):
            ...
            pred_list = torch.chunk(outputs, num_models)

            outputs = sum(i for i in pred_list) / num_models
